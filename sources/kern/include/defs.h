...
//
//
// $Id: defs.h,v 1.2 2007/03/13 17:29:14 hoshina Exp $
// GEANT4 tag $Name:  $
//
// 
//---------------------------------------------------------------
...
public:
  ...
  G4RWBoundsErr(const char* st,int b=0,int v=0)
    {
      char btmp[80],vtmp[80];
      sprintf(btmp,"%d",b);
      sprintf(vtmp,"%d",v);
      str=new char[strlen(st)+8+strlen(btmp)+7+strlen(vtmp)+1];
      strcpy(str,st);
      strcat(str,": bound:");
      strcat(str,btmp);
      strcat(str," value:");
...
public:
  ...
  G4RWGeneralException(const char* st)
    {
